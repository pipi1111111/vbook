...
import (
	...
	"vbook/interactive/integration/startup"
...
func (s *InteractiveTestSuite) TestGet() {
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			tc.before(t)
			res, err := svc.Get(context.Background(), &interv1.GetRequest{
				Biz: tc.biz, BizId: tc.bizId, Uid: tc.uid,
			})
			assert.Equal(t, tc.wantErr, err)
...
func (s *InteractiveTestSuite) TestGetByIds() {
	...
	testCases := []struct {
		...
		wantErr error
		wantRes *interv1.GetByIdsResponse
	}{
		{
			name: "查找成功",
			biz:  "test",
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			res, err := svc.GetByIds(context.Background(), &interv1.GetByIdsRequest{
